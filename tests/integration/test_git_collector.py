...
def make_git_repo():
    ...
    def run(*args):
        subprocess.run(list(args), cwd=tmp, capture_output=True, check=True, env=env)
    ...
    subprocess.run(
        ["git", "-C", str(tmp), "commit", "-m", "feat: initial commit"],
        capture_output=True,
        check=True,
        env=env,
    )
    ...
    subprocess.run(["git", "-C", str(tmp), "add", "."], capture_output=True, check=True)
    subprocess.run(
        ["git", "-C", str(tmp), "commit", "-m", "fix: add world module"],
        capture_output=True,
        check=True,
        env=env,
    )
...
REPO = make_git_repo()
SINCE_OLD = datetime(2024, 1, 1, tzinfo=UTC)
...
class TestGitIntegration(unittest.TestCase):
    def test_collects_commits(self):
        ...
        assert any("initial commit" in s for s in summaries)
        assert any("world module" in s for s in summaries)
    ...
    def test_since_filters_old_commits(self):
        tl = build_timeline(str(REPO), datetime.now(tz=UTC))
        assert tl.is_empty
    ...
    def test_invalid_repo_raises(self):
        ...
        with pytest.raises(RuntimeError):
            build_timeline(tempfile.mkdtemp(), datetime.now(tz=UTC))
